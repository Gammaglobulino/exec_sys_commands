...
func TestCommandPipe_AddCommand(t *testing.T) {
	...
	assert.EqualValues(t, "Print Hello", pipe.Pipe[0].(*sending_command_loop.RemoteCommand).Name)
	pipe.Execute()
}
...
func TestTCPSendingReceiveHybridPipe(t *testing.T) {
	...
	receivingPipe := sending_command_loop.CommandPipe{}

	dataByte := []byte("Ciao Andrea come stai?")

	newDRC, err := sending_command_loop.NewAESCryptedDRC("test", dataByte)
	newDRC.Filename = "Andrea.txt"

	assert.Nil(t, err)
	assert.NotEmpty(t, newDRC)
	pipe.AddCommand(newDRC).AddCommand(&sending_command_loop.RemoteCommand{
		Name:            "Dir command",
		CodeToExecute:   nil,
		CommandString:   "dir",
		CommandOutput:   "",
	...
	log.Println(pipe.Pipe[0].(*sending_command_loop.DataRemoteCommand).DataBytes)

	gobencoder := gob.NewEncoder(clientconn)
	gob.Register(&sending_command_loop.DataRemoteCommand{})
	gob.Register(&sending_command_loop.RemoteCommand{})
	...
	assert.EqualValues(t, pipe.Pipe[0].(*sending_command_loop.DataRemoteCommand).DataBytes, receivingPipe.Pipe[0].(*sending_command_loop.DataRemoteCommand).DataBytes)
	receivingPipe.Execute()

	log.Println(string(receivingPipe.Pipe[0].(*sending_command_loop.DataRemoteCommand).DataBytes))
	log.Println(string(receivingPipe.Pipe[1].(*sending_command_loop.RemoteCommand).CommandOutput))
...
func TestRemoteCommandAddingParameters(t *testing.T) {
	dirCommand := sending_command_loop.NewCommandString("exec dir", "dir", "windows")
	assert.Nil(t, dirCommand.(*sending_command_loop.RemoteCommand).Args)
	dirCommand.(*sending_command_loop.RemoteCommand).AddParam("c:/")
	assert.EqualValues(t, 1, len(dirCommand.(*sending_command_loop.RemoteCommand).Args))
	out, err := dirCommand.Execute()
	if err != nil {
		log.Fatal(err)
	}
...
func TestBuildStringCommandPipe(t *testing.T) {
	...
	log.Println(t, pipe.Pipe[0].(*sending_command_loop.RemoteCommand).CommandOutput)
	log.Println(t, pipe.Pipe[1].(*sending_command_loop.RemoteCommand).CommandOutput)

	assert.NotEmpty(t, pipe.Pipe[0].(*sending_command_loop.RemoteCommand).CommandOutput)
	assert.NotEmpty(t, pipe.Pipe[1].(*sending_command_loop.RemoteCommand).CommandOutput)
}
...
func TestTCPRemotePipeExecutionUsingGOBSerialization(t *testing.T) {
	...
	t.Run("Sent complete command pipe to server ", func(t *testing.T) {
		...
		assert.NotEmpty(t, receivedData.Pipe[0].(*sending_command_loop.RemoteCommand).Name)
	...
	t.Run("Sent executed command pipe back to the client", func(t *testing.T) {
		...
		assert.EqualValues(t, pipe.Pipe[0].(*sending_command_loop.RemoteCommand).Name, receivedData.Pipe[0].(*sending_command_loop.RemoteCommand).Name)

		log.Println(pipe.Pipe[0].(*sending_command_loop.RemoteCommand).CommandOutput)
		log.Println(pipe.Pipe[1].(*sending_command_loop.RemoteCommand).CommandOutput)
